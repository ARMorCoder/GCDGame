    ...
    void SpawnObstacleOverTime(){
        ...
        IEnumerator SpawnObstacleOverTimeRoutine(){
            while(true){
                ...
                //GameObject newUFO = Instantiate(tankPrefab, new Vector3(Location1.position.x, Location1.position.y, 0), Quaternion.identity);
                GameObject newTank = Instantiate(tankPrefab, new Vector3(newLocate.position.x, newLocate.position.y, 0), Quaternion.identity);
                newTank.GetComponent<TankHandler>().target = player.transform;
                newTank.GetComponent<LandAIChase>().player = player;
